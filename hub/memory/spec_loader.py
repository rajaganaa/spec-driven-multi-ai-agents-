...
from hub.memory import vector_store
...
def _find_feature_spec_path(feature_id: Optional[str]) -> Optional[Path]:
    """Locate a feature spec file by id (e.g. 'F01'), mirroring
    find_task_spec_path's prefix-match approach."""
    if not feature_id or not FEATURES_DIR.exists():
        return None
    matches = sorted(p for p in FEATURES_DIR.glob("*.md") if p.stem.lower().startswith(feature_id.lower()))
    return matches[0] if matches else None


def _read_attempt_logs(project_id: str, task_id: str) -> List[dict]:
    """Best-effort read of every attempt log already persisted for this
    task by hub/runner/task_runner.py's _persist_attempt_log (under
    workspace/projects/<project_id>/.agent_logs/<task_id>/*.json). Used
    only for RAG indexing below — returns [] on any error, since a
    logging read failure must never block a status update."""
    listing = tools.list_dir(project_id, f".agent_logs/{task_id}")
    ...
    for item in listing.get("entries", []):
        if item.get("type") != "file" or not item.get("path", "").endswith(".json"):
            continue
        read = tools.read_file(project_id, f".agent_logs/{task_id}/{item['path']}")
        if not read.get("ok"):
            continue
        try:
            logs.append(json.loads(read["content"]))
        except (json.JSONDecodeError, TypeError):
            continue
    return logs


def _index_completed_task(task_id: str, feature: Optional[str], commit: Optional[str]) -> None:
    """Index a just-completed task's spec, feature spec, final code
    diff, and any Reviewer/Tester feedback into hub/memory/vector_store.py
    so future tasks' specialists can retrieve relevant past work (see
    hub/agents/specialists/common.py's run_specialist_task).

    Called from update_board_status() below whenever status flips to
    'done' — this is the intended integration point for Phase 2's RAG
    memory: hub/runner/task_runner.py is the only caller that ever
    passes status='done', but this hook lives here (not in
    task_runner.py itself) so task_runner.py needs zero changes and
    stays exactly as it was for Phase 1.

    Gated on config/agents.yaml's rag.enabled (default True) and
    entirely best-effort: any failure here (missing chromadb data
    directory, embedding error, git not available, etc.) is swallowed —
    indexing memory must never be able to fail a task that otherwise
    succeeded.

    NOTE on "completed feature specs": a feature is made up of several
    tasks, and this hook only observes one task completing at a time,
    with no reliable signal here for "every task in this feature is
    now done." So the feature spec is (re-)indexed each time any of its
    tasks completes. This is idempotent (deterministic entry_id, so a
    re-index overwrites rather than duplicates) and means the feature
    spec is typically already indexed well before the feature is fully
    complete — a reasonable trade given task_runner.py can't be
    modified to add an explicit "feature done" signal."""
    try:
        ...
        if spec_path is not None:
            try:
                task_text = spec_path.read_text(encoding="utf-8")
            except OSError:
                task_text = None
            if task_text and task_text.strip():
                vector_store.index_entry(
                    project_id, "task_spec", task_text,
                    task_id=task_id, entry_id=f"{project_id}:{task_id}:task_spec",
                )

        feature_spec_path = _find_feature_spec_path(feature)
        if feature_spec_path is not None:
            try:
                feature_text = feature_spec_path.read_text(encoding="utf-8")
            except OSError:
                feature_text = None
            if feature_text and feature_text.strip():
                vector_store.index_entry(
                    project_id, "feature_spec", feature_text,
                    task_id=task_id, entry_id=f"{project_id}:{feature}:feature_spec",
                )

        if commit:
            diff_result = tools.run_terminal(project_id, f"git show {commit}")
            diff_text = (diff_result.get("stdout") or "") if diff_result.get("ok") else ""
            if diff_text.strip():
                vector_store.index_entry(
                    project_id, "code_diff", diff_text,
                    task_id=task_id, entry_id=f"{project_id}:{task_id}:code_diff:{commit}",
                )

        for log in _read_attempt_logs(project_id, task_id):
            role = log.get("role")
            final_text = log.get("final_text")
            if role not in ("reviewer", "tester") or not final_text or not str(final_text).strip():
                continue
            kind = "review_feedback" if role == "reviewer" else "test_feedback"
            attempt = log.get("attempt", "unknown")
            vector_store.index_entry(
                project_id, kind, str(final_text),
                task_id=task_id, role=role,
                entry_id=f"{project_id}:{task_id}:feedback:{role}:{attempt}",
            )
    except Exception:
        # Best-effort, same principle as task_runner.py's
        # _persist_attempt_log: indexing memory must never fail the task.
        pass
...
def update_board_status(
    ...
    if status == "done":
        _index_completed_task(task_id, entry.get("feature"), entry.get("commit"))
